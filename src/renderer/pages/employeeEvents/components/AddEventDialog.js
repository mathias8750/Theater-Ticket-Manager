...
import { Dialog, DialogTitle, TextField, Typography, Select, MenuItem, Button, InputLabel, FormControl } from "@mui/material";
...
const AddEventDialog = ({open, onClose, fetchEvents}) => {
    ...
    const insertEvent = async () => {
        ...
        const {data: Events, error} = await supabase
        .from('Events')
        .insert([{seasonID: season, organizationID: state.selectedOrg.organizationID, venueID: eventVenueID, eventDateTime: dt.toString(), eventName: eventNameRef.current.value.trim()}]);
